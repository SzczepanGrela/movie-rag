...
async def _fake_dispatch(session: Any, embedder: Any, name: str, args: dict[str, Any],
                         cited: dict[int, dict[str, Any]]) -> Any:
...
async def test_explain_stream_tool_then_answer() -> None:
    script: list[list[TurnEvent]] = [
        [ToolCallsReady([ToolCall(id="c1", name="search_movies",
                                  arguments={"semantic_query": "plane"})])],
        [ContentDelta("The film is "), ContentDelta("**Con Air**.")],
    ]
    out = await _collect(
        service.explain_stream(cast(AsyncSession, None), cast(Embedder, None),
                               _ScriptedProvider(script), "plane",
    ...
    assert (out.index("tool_call") < out.index("chunk")
            < out.index("cited_movie") < out.index("done"))
...
async def test_explain_stream_max_iterations() -> None:
    ...
    forever: list[list[TurnEvent]] = [
        [ToolCallsReady([ToolCall(id=f"c{i}", name="search_movies",
                                  arguments={"semantic_query": "x"})])]
        for i in range(10)
    ]
    out = await _collect(
        service.explain_stream(cast(AsyncSession, None), cast(Embedder, None),
                               _ScriptedProvider(forever), "x",
                               dispatch=_fake_dispatch, max_iterations=3)
    )
...
async def test_explain_stream_rate_limited() -> None:
    out = await _collect(
        service.explain_stream(cast(AsyncSession, None), cast(Embedder, None),
                               _RaisingProvider(), "x", dispatch=_fake_dispatch)
    )
