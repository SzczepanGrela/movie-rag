...
def test_fake_embedder_deterministic() -> None:
    ...
    [v1] = embedder.embed(["the cat sat on the mat"], kind="document")
    [v2] = embedder.embed(["the cat sat on the mat"], kind="document")
    assert v1 == v2


def test_fake_embedder_correct_shape() -> None:
    embedder = FakeEmbedder()
    result = embedder.embed(["a", "b", "c"], kind="document")
    assert len(result) == 3
...
def test_fake_embedder_different_texts_differ() -> None:
    embedder = FakeEmbedder()
    [va, vb] = embedder.embed(["alpha", "beta"], kind="query")
    assert va != vb


def test_fake_embedder_normalized() -> None:
    embedder = FakeEmbedder()
    [vec] = embedder.embed(["some text to embed"], kind="document")
...
async def test_embed_async_uses_thread() -> None:
    embedder = FakeEmbedder()
    result = await embed_async(embedder, ["x", "y"], kind="query")
    assert len(result) == 2
    assert result == embedder.embed(["x", "y"], kind="query")
